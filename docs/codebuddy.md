...
以下结论来自 webview 沙箱等价环境的实测（IDE 内置预览 = sandbox 化 iframe），并已在 **VS Code Simple Browser 真机**复核（同为 VS Code 系 webview，与 CodeBuddy IDE 预览同源架构）：

- **标准 IDE 预览沙箱（含 `allow-same-origin`）下全功能可用**：Shadow DOM 工具栏、hover 高亮、点选选框与 8 向手柄、属性面板（Auto layout / Appearance / Fill / Stroke 数值读取正确）、拖拽吸附、撤销重做、localStorage 会话持久化、导出，全部正常。
